...
class DrawerRootVC: UIViewController {
    // 预加载视图控制器
    ...
    //    打开视图
    //
    func openWithGestureCompletelyAnimated(_ animated: Bool) {
        if self.isOpen {
            return;
        }
        if animated {
    ...
    func openWithManualCompletelyAnimated(_ animated: Bool) {
        if self.isOpen {
            return;
        }
        if animated {
            UIView.animate(withDuration: OpenAnimationTime, animations: {
                self.rootViewController.view.frame = CGRect(x: CGFloat(self.maxOffsetX()), y: 0, width: self.view.bounds.size.width, height: self.view.bounds.size.height)
            }, completion: { (finished:Bool) in
                self.panGesture?.isEnabled = (self.isOpen && finished)
            })
        }else {
    ...
    //    tap点击事件
    func gestureRecognizerDidTap(_ sender:UITapGestureRecognizer) {
        UIView.animate(withDuration: OpenAnimationTime, animations: {
            self.rootViewController.view.frame = self.getRect(0)
        });
    }
    ...
    //    pan手势事件
    func gestureRecognizerDidPan(_ sender:UIPanGestureRecognizer) {
